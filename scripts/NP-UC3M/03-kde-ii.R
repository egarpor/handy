...
## -----------------------------------------------------------------------------
## Name: 03-kde-ii.R
## Description: Script for Chapter 3 of "Nonparametric Statistics"
...
# Assign colors automatically from quantiles to have an idea of the densities of
# each one
n_cols <- 20
...
## ----kde-3d, cache = TRUE, eval = knitr:::is_html_output()--------------------
...
# generate an error, but they return a nonsense kde
...
# H positive semidefinite but not positive definite
H <- rbind(c(1.5, 0.25, 0.5),
           c(0.25, 0.5, 1),
           c(0.5, 1, 2))
eigen(H)$values
...
# The geometry::inhulln() allows checking if points are inside the convex hull
...
plot(nor1mix::MW.nm10, main = "", p.norm = FALSE)
